...
function useQueryParams() {
  ...
  return [params, (next: URLSearchParams) => {
...
export default function SearchPage() {
  ...
  useEffect(() => {
    search({ q, section: section || undefined, tags: tags.length ? tags : undefined, kind: kind || undefined, sort, page, perPage })
      .then(setResult);
  ...
  function toggleTag(tag: string) {
    ...
    if (updated.length) next.set("tags", updated.join(",")); else next.delete("tags");
    next.delete("page");
    setParams(next);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch">
        ...
        />
        <select className="px-3 py-2 rounded border" value={sort} onChange={(e) => update("sort", e.target.value)}>
        ...
        <button
          ...
          onClick={() => {
            const input = document.querySelector("input[placeholder='Search charts…']") as HTMLInputElement;
      ...
      <div className="grid md:grid-cols-4 gap-6">
        <aside className="md:col-span-1 space-y-4">
          <div className="card">
            ...
            <select className="w-full px-2 py-2 rounded border" value={section} onChange={(e) => update("section", e.target.value)}>
          ...
          <div className="card">
            ...
            <select className="w-full px-2 py-2 rounded border" value={kind} onChange={(e) => update("kind", e.target.value)}>
              <option value="">All</option>
          ...
          <div className="card">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(result?.facets.tags ?? {}).sort((a,b)=>b[1]-a[1]).slice(0, 12).map(([k, c]) => {
                const active = tags.includes(k);
                return (
                  <button
                    key={k}
                    onClick={() => toggleTag(k)}
                    className={"text-sm px-2 py-1 rounded border " + (active ? "bg-white/10" : "")}
                    title={`${c} results`}
                  >{k}</button>
                );
              })}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {result.hits.map(m => (
                  <Link key={m.slug} href={`/charts/${m.slug}`} className="card hover:shadow">
                    ...
                    <div className="text-sm opacity-70 mt-2">{new Date(m.publishedAt).toLocaleDateString()}</div>
                    <div className="text-xs opacity-70 mt-2">{m.tags.join(", ")}</div>
                  </Link>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <button className="px-3 py-1 rounded border" disabled={(result.page<=1)} onClick={() => goto(result.page-1)}>Prev</button>
                <div className="opacity-80 text-sm">Page {result.page} / {Math.max(1, Math.ceil(result.total / result.perPage))}</div>
                <button className="px-3 py-1 rounded border" disabled={(result.page*result.perPage>=result.total)} onClick={() => goto(result.page+1)}>Next</button>
              </div>
            </>
          )}
        </section>
